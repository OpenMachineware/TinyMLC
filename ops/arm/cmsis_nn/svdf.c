 ...
 * SVDF operator
 *
 * Uses simple implementation for now. CMSIS-NN has a more complex
 * version with state management, but for TFLite micro compatibility
 * we use the simple implementation.
 */
    ...
    for (int t = 0; t < time_steps; t++) {
        ...
        for (int i = 0; i < output_size; i++) {
            ...
            for (int j = 0; j < input_size; j++) {
                sum += (int32_t)input_ptr[j] *
                       (int32_t)weights[i * input_size + j];
            }
