...
void tmlc_global_avg_pool_s8(const int8_t* input, int8_t* output,
    ...
    arm_nn_per_channel_quant_params quant_params;
    arm_nn_init_per_channel_quant_params(&quant_params, 1.0f);

    arm_avgpool_s8(input, output, H, W, C, &pool_args, &quant_params);
}
