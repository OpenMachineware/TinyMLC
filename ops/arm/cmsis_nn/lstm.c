 ...
 * LSTM operator
 *
 * Uses the same implementation as the pure C version.
 * CMSIS-NN has an optimized LSTM but with different interface,
 * so for now we use the simple implementation for compatibility.
 */
    ...
    for (int t = 0; t < time_steps; t++) {
        for (int i = 0; i < hidden_size; i++) {
            ...
            for (int j = 0; j < input_size; j++) {
                sum += (int32_t)x_ptr[j] *
                       (int32_t)wi_ptr[i * input_size + j];
            }
            for (int j = 0; j < hidden_size; j++) {
                sum += (int32_t)h_cur[j] *
                       (int32_t)ri_ptr[i * hidden_size + j];
            }
            gate_i[i] = sum;
        }
        ...
        for (int i = 0; i < hidden_size; i++) {
            ...
            for (int j = 0; j < input_size; j++) {
                sum += (int32_t)x_ptr[j] *
                       (int32_t)wf_ptr[i * input_size + j];
            }
            for (int j = 0; j < hidden_size; j++) {
                sum += (int32_t)h_cur[j] *
                       (int32_t)rf_ptr[i * hidden_size + j];
            }
            gate_f[i] = sum;
        }
        ...
        for (int i = 0; i < hidden_size; i++) {
            ...
            for (int j = 0; j < input_size; j++) {
                sum += (int32_t)x_ptr[j] *
                       (int32_t)wg_ptr[i * input_size + j];
            }
            for (int j = 0; j < hidden_size; j++) {
                sum += (int32_t)h_cur[j] *
                       (int32_t)rg_ptr[i * hidden_size + j];
            }
            gate_g[i] = sum;
        }
        ...
        for (int i = 0; i < hidden_size; i++) {
            ...
            for (int j = 0; j < input_size; j++) {
                sum += (int32_t)x_ptr[j] *
                       (int32_t)wo_ptr[i * input_size + j];
            }
            for (int j = 0; j < hidden_size; j++) {
                sum += (int32_t)h_cur[j] *
                       (int32_t)ro_ptr[i * hidden_size + j];
            }
            gate_o[i] = sum;
        }
