/**
 * NMSIS-NN accelerated depthwise convolution operator
 *
 * Wraps riscv_depthwise_conv_s8 to match TinyMLC's
 * tmlc_depthwise_conv_2d_s8 interface.
 */
                               ...
                               int32_t multiplier, int32_t shift)
{
    ...
    nmsis_nn_dims input_dims = {.n = 1, .h = input_h, .w = input_w, .c = input_c};
    nmsis_nn_dims filter_dims = {.n = 1, .h = kernel_h, .w = kernel_w, .c = input_c * depth_multiplier};
    nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = output_c};
    nmsis_nn_dims output_dims = {.n = 1, .h = output_h, .w = output_w, .c = output_c};

    ctx.buf = nmsis_nn_buf;
    ctx.size = sizeof(nmsis_nn_buf);

    riscv_depthwise_conv_s8(&ctx, &dw_conv_params, &quant_params,
                            &input_dims, input,
                            &filter_dims, weights,
                            &bias_dims, bias,
                            &output_dims, output);
